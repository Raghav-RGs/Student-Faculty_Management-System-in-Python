    ...
    def insert_std():
        ...
        if (con):
            ...
            try:
                ...
                pre_qry = "insert into student values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                cur.execute(pre_qry, (iD,Passd,Name[0],Name[1],br,sem,sec,add,city,fees,mail,att))
                con.commit()
                if cur.rowcount != 0:

                        pw.wm_iconbitmap("conn.ico")
                        messagebox.showinfo("Update info", "New Student Added")
                        clearall()
                        pw.destroy()
                else:
                    pw.wm_iconbitmap("error.ico")
                    messagebox.showinfo("Update info", "Insertion Failed")
                    clearall()
                    pw.destroy()
            except:
                    pw.wm_iconbitmap("error.ico")
