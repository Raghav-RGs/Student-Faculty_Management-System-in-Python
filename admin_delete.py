...
def delete_ad(pr_win):
    ...
    def check():
        ...
        if (con):
            ...
            ''' Super Admin password=Raghav@123'''
            #print(name)
            if name[0] == 'Raghav':
                pw.wm_iconbitmap('error.ico')
                messagebox.showinfo('Invalid Action', message="You cannot delete SuperAdmin")
                ename.delete(0, END)
                epass.delete(0, END)
            elif 'Raghav@123' == passw:
                cur = con.cursor(prepared=True)
                try:
                    qry = 'delete from admin where Username=%s'
    ...
    pw = Toplevel(pr_win)
    pw.title('Delete Admin')
    pw.geometry("600x400+450+200")
    heading = Label(pw, text='Delete admin', font=('Calibri', 36), border=1)
