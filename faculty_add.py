...
def faculty_add(pr_win):
        ...
        def insert_fa():
            ...
            if (con):
                ...
                try:
                    ...
                    pre_qry = "insert into faculty values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                    cur.execute(pre_qry, (id, Fname, Lname, br, sec, sub, passwd, sem, city))
                    con.commit()
                    if cur.rowcount != 0:


                            pw.wm_iconbitmap("conn.ico")
                            messagebox.showinfo("Update info", "New Faculty Added")
                            clearall()
                            pw.destroy()
                    else:
        ...
        heading = Label(pw, text=" ADD NEW FACULTY", font=('Calibri', 36))
